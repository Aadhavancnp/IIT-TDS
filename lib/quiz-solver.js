  ...
  try {
    while (currentUrl) {
      ...
      try {
        ...
        if (
          currentUrl.includes("gh-tree") ||
          quiz.question.includes("GitHub API") ||
          quiz.question.includes("git/trees")
        ) {
          console.log("\n🔧 GitHub API Quiz Detected");
          const result = await handleGitHubApiQuiz(
            quiz,
            email,
            secret,
            currentUrl
        ...
        // Handle Heatmap quiz
        if (
          currentUrl.includes("heatmap") ||
          quiz.question.includes("heatmap") ||
          quiz.question.includes("RGB color")
        ) {
          console.log("\n🎨 Heatmap Quiz Detected");
          const result = await handleHeatmapQuiz(
            quiz,
            email,
            secret,
            currentUrl
        ...
        if (
          currentUrl.includes("project2-csv") ||
          quiz.question.includes("messy.csv") ||
          quiz.question.includes("snake_case")
        ) {
          console.log("\n📊 CSV Normalization Quiz Detected");
          const result = await handleCsvNormalizationQuiz(
            quiz,
            email,
            secret,
            currentUrl
        ...
        if (
          currentUrl.includes("audio") ||
          quiz.question.includes("audio") ||
          quiz.question.includes("Transcribe")
        ) {
...
async function handleGitHubApiQuiz(quiz, email, secret, currentUrl) {
  ...
  try {
    ...
    const jsonUrl =
      "https://tds-llm-analysis.s-anand.net/project2/gh-tree.json";
    ...
    console.log(
      `🔍 Looking for files: prefix="${targetPrefix}", extension="${targetExtension}"`
    );

    const matchingFiles = tree.filter((item) => {
      if (item.type !== "blob") return false;

      const matchesPrefix = targetPrefix
    ...
    console.log(
      `📧 Email personalization: ${email.length} mod 2 = ${personalizationOffset}`
    );

    // Final answer = count + offset
    const answer = matchingFiles.length + personalizationOffset;
    console.log(
      `✅ Final answer: ${matchingFiles.length} + ${personalizationOffset} = ${answer}`
    );

    // Submit answer
    const submitUrl = "https://tds-llm-analysis.s-anand.net/submit";
    const submitQuizUrl =
      "https://tds-llm-analysis.s-anand.net/project2-gh-tree";
...
async function handleHeatmapQuiz(quiz, email, secret, currentUrl) {
  ...
  try {
    ...
    const imageUrl =
      "https://tds-llm-analysis.s-anand.net/project2/heatmap.png";
    ...
    try {
      ...
      console.log(
        `🖼️ Image size: ${info.width}x${info.height}, channels: ${info.channels}`
      );
      ...
      console.log(
        "⚠️ Sharp failed, using LLM for image analysis:",
        sharpError.message
      );
      // Fallback to LLM analysis
      hexColor = await analyzeImage(
        imageBuffer,
        "Find the most frequent RGB color in this heatmap image and return it as a hex string like #rrggbb"
      );
    }
    ...
    const submitUrl = "https://tds-llm-analysis.s-anand.net/submit";
    const submitQuizUrl =
      "https://tds-llm-analysis.s-anand.net/project2-heatmap";
...
async function handleAudioQuiz(quiz, email, secret, currentUrl) {
  ...
  try {
    ...
    const audioUrl =
      "https://tds-llm-analysis.s-anand.net/project2/audio-passphrase.opus";
    ...
    const submitUrl = "https://tds-llm-analysis.s-anand.net/submit";
    const submitQuizUrl =
      "https://tds-llm-analysis.s-anand.net/project2-audio-passphrase";
